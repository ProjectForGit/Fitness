...
class EctomorphActivity : AppCompatActivity() {
    ...
    fun Calculation() {
        ...
        {
            if (woman.isChecked) {
                value = (447.7 + (9.2 * weightText) + (3.1 * heightText) - (4.3 * ageText)) * 1.4
                kcal.setText(value.toInt().toString())
            }
            if (man.isChecked) {
                value = (88.36 + (13.4 * weightText) + (4.8 * heightText) - (5.7 * ageText)) * 1.7
                kcal.setText(value.toInt().toString())
            }
        }
        else if (sportsmanTarget == "Похудение")
        {
            ...
            if (man.isChecked) {
                value = (88.36 + (13.4 * weightText) + (4.8 * heightText) - (5.7 * ageText)) * 1.4
                kcal.setText(value.toInt().toString())
            }
        }
